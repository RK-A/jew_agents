...
class AgentOrchestrator:
    ...
    async def get_task_type(self, message, conversation_history) -> str:
        ...
        1. "taste": Запросы на определение стиля пользователя, получение овета на вопрос, выявление предпочтений, диагностика вкуса.
        2. "consultation": Вопросы о конкретных товарах, материалах (золото, серебро, пробы), характеристиках, весе, размерах, наличии, доставке или цене. Фактические вопросы.
        3. "girlfriend": Запросы совета по стилю ("подойдет ли мне?", "с чем носить?"), просьба высказать мнение, эмоциональное общение, поддержка выбора подарка, "подружка".
